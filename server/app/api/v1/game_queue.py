...
async def remove_player():
    ...
    if len(curr_players) == 0:
        total_players_in_queue = sum(player.get("groupSize", 1) for player in queue)
        ...
        if total_players_in_queue >= 4:
            try:
                ...
                group_of_two = []
                group_of_three = []
                curr_len_players = sum(player.get("groupSize", 1) for player in curr_players)
                ...
                if remaining_spots == 0:
                    curr_players.append(first_player)  # Append the player object
                    queue.pop(0)  # Remove the player from the queue
                ...
                if remaining_spots == 1:
                    if len(group_of_one) != 0:
                        curr_players.append(first_player)  # Append the player object
                        curr_players.append(group_of_one[0])  # Append the player object
                        queue.pop(0)  # Remove the first player from the queue
                        queue.remove(group_of_one[0])  # Remove the group of one from the queue
                        print(f"Added {first_player['name']} and {group_of_one[0]['name']} to curr_players")

                elif remaining_spots == 2:
                    if len(group_of_two) >= 1:
                        curr_players.append(first_player)  # Append the player object
                        curr_players.append(group_of_two[0])  # Append the player object
                        queue.pop(0)  # Remove the first player from the queue
                        queue.remove(group_of_two[0])  # Remove the group of two from the queue
                        print(f"Added {first_player['name']} and {group_of_two[0]['name']} to curr_players")

                    elif len(group_of_one) >= 2:
                        curr_players.append(first_player)  # Append the player object
                        curr_players.append(group_of_one[0])  # Append the player object
                        curr_players.append(group_of_one[1])  # Append the player object

                        queue.pop(0)  # Remove the first player from the queue
                        queue.remove(group_of_one[0])  # Remove the group of one from the queue
                        queue.remove(group_of_one[1])  # Remove the second group of one from the queue
                        print(f"Added {first_player['name']}, {group_of_one[0]['name']}, and {group_of_one[1]['name']} to curr_players")

                elif remaining_spots == 3:
                    if len(group_of_three) != 0:
                        curr_players.append(first_player)  # Append the player object
                        curr_players.append(group_of_three[0])  # Append the player object
                        queue.pop(0)  # Remove the first player from the queue
                        queue.remove(group_of_three[0])  # Remove the group of three from the queue
                        print(f"Added {first_player['name']} and {group_of_three[0]['name']} to curr_players")
                        ...
                        queue.remove(group_of_one[0])  # Remove the group of one from the queue
                        queue.remove(group_of_two[0])  # Remove the group of two from the queue
                        print(f"Added {first_player['name']}, {group_of_one[0]['name']}, and {group_of_two[0]['name']} to curr_players")

                    elif len(group_of_one) >= 3:
                        curr_players.append(first_player)  # Append the player object
                        curr_players.append(group_of_one[0])  # Append the player object
                        curr_players.append(group_of_one[1])  # Append the player object
                        curr_players.append(group_of_one[2])  # Append the player object

                        queue.pop(0)  # Remove the first player from the queue
                        queue.remove(group_of_one[0])  # Remove the first group of one from the queue
                        queue.remove(group_of_one[1])  # Remove the second group of one from the queue
                        queue.remove(group_of_one[2])  # Remove the third group of one from the queue
                        print(f"Added {first_player['name']}, {group_of_one[0]['name']}, {group_of_one[1]['name']}, and {group_of_one[2]['name']} to curr_players")
...
async def get_queue():
    ...
    total_count = sum(player.get("groupSize", 1) for player in queue)
    return {
        "queue": queue,
...
async def add_to_queue(player: Player):
    ...
    try:
        ...
        queue.append({"name": player.name, "phoneNumber": player.phoneNumber, "groupSize": player.groupSize})
        position = sum(player.get("groupSize", 1) for player in queue)
